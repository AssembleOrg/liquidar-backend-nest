...
export class EmailService {
  ...
  constructor(private configService: ConfigService) {
    ...
    // Only set API key if it's not a dummy key
    if (apiKey && apiKey !== 'dummy_key_for_development') {
      ...
      this.logger.warn('Using dummy API key - emails will not be sent in development mode');
    }
  ...
  async sendVerificationEmail(email: string, firstName: string, verificationToken: string): Promise<void> {
    ...
    try {
      ...
      if (apiKey === 'dummy_key_for_development') {
        ...
        this.logger.log(`[DEV MODE] Email content: ${this.sendSmtpEmail.htmlContent?.substring(0, 200)}...`);
        return;
      }
